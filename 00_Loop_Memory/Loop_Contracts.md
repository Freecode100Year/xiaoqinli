...
| **`compiler` 性能基准测试套件** | `compiler/bench_test.go` | **已建立全量 Benchmark 基线 (Benchmarked)** | 新增 `BenchmarkCompile_500Fn`、`BenchmarkValidate_500Fn` 和 `BenchmarkCompile_ScalingFn` 性能基准断言，可单命令（`go test ./compiler -bench=.`）防范编译性能回退。 |
